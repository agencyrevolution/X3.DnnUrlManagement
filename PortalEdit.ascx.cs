    ...
    public partial class PortalEdit : PortalModuleBase
    {
        ...
        private void LoadSettings()
        {
            try
            {
                ...
                ListItem liDeletedTabHandling = rblDeletedTabHandlingType.Items.FindByValue(((int)fs.DeletedTabHandlingType).ToString());
                ...
                if (fs.TabId404 > 0)
                {
                    ddlErrorPage404.SelectedPage = tabs.Where(i => i.TabID == fs.TabId404).SingleOrDefault();
                }

                // AUM_ErrorPage500
                if (fs.TabId500 > 0)
                {
                    ddlErrorPage500.SelectedPage = tabs.Where(i => i.TabID == fs.TabId500).SingleOrDefault();
                }
        ...
        protected void gvCustomUrls_ItemCreated(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridEditableItem && (e.Item.IsInEditMode))
            {
                ...
                DropDownList ddlPortalAliasID = (DropDownList)editableItem["PortalAliasId"].FindControl("ddlPortalAliasId");
        ...
        protected void gvCustomUrls_InsertCommand(object sender, GridCommandEventArgs e)
        {
            ...
            tabUrl.PortalAliasUsage = PortalAliasUsageType.Default;
            if (ht["Url"] != null)
            {
